...
function getElementInput(element, field, type) {
  ...
  return !type || type === "string" ? value2 : type === "string[]" ? options ? [
    ...
    ...field.value || [],
    value2
  ] : (field.value || []).filter((v) => v !== value2) : type === "number" ? valueAsNumber : type === "boolean" ? checked : type === "File" && files ? qwik.noSerialize(files[0]) : type === "File[]" && files ? qwik.noSerialize([
    ...files
  ]) : type === "Date" && valueAsDate ? valueAsDate : field.value;
}
...
function getFormDataValues(formData, formDataInfo = {}) {
  ...
  const values = [
    ...
    const getDate = () => (
      // Date (yyyy-mm-dd)
      /^\d{4}-(0[1-9]|1[0-2])-([12]\d|0[1-9]|3[01])$/.test(value2) ? /* @__PURE__ */ new Date(`${value2}T00:00:00.000Z`) : /^\d{4}-(0[1-9]|1[0-2])-([12]\d|0[1-9]|3[01])T(1\d|0[1-9]|2[0-3]):[0-5]\d$/.test(value2) ? /* @__PURE__ */ new Date(`${value2}:00.000Z`) : /^\d{4}-W(0[1-9]|[1-4]\d|5[0-3])$/.test(value2) ? (() => {
        const [year, week] = value2.split("-W");
        const date = /* @__PURE__ */ new Date(`${year}-01-01T00:00:00.000Z`);
        date.setUTCDate((+week - 1) * 7 + 1);
        return date;
      })() : /^(1\d|0[1-9]|2[0-3]):[0-5]\d$/.test(value2) ? /* @__PURE__ */ new Date(`1970-01-01T${value2}:00.000Z`) : /^(1\d|0[1-9]|2[0-3]):[0-5]\d:[0-5]\d$/.test(value2) ? /* @__PURE__ */ new Date(`1970-01-01T${value2}.000Z`) : new Date(value2)
    );
  ...
  return qwikCity.globalActionQrl(/* @__PURE__ */ qwik.inlinedQrl(async (jsonData, event) => {
    ...
    if (!Object.keys(errors).length)
      try {
        ...
        if (error instanceof requestHandler.AbortMessage)
          throw error;
        else {
          console.error(error);
